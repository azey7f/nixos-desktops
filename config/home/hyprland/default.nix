...
in {
  ...
  options.az.desktop.environment.hyprland = with azLib.opt; {
      ...
      type = with types;
        attrsOf (submodule ({name, ...}: {
          options = {
            ...
            resolution = optStr "preferred";
            position = optStr "auto";
            scale = mkOpt numbers.positive 1;
            extraArgs = optStr "";
          };
        }));
      default = {};
    ...
    inputDevices = mkOption {
      type = with types;
        attrsOf (submodule ({name, ...}: {
          freeformType = attrsOf (oneOf [str number bool]);
          options.name = optStr name;
        }));
      default = {};
    };
  ...
  config = mkIf cfg.enable {
    ...
    az.desktop.environment.hyprland.monitors."" = mkDefault {
